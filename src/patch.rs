use std::{fs, fs::{File}, io::{BufReader, Error, ErrorKind, Result, Read, Write}, path::PathBuf, collections::{HashMap, HashSet}};
    ...
    pub fn from(input_path: &PathBuf) -> Result<Self> {
        ...
        for i in 0..zip.len(){
            let Ok(mut entry) = zip.by_index(i) else {
                println!("Ignored file indexed by {} in {:?} due to errors while reading", i, zip_file_name);
                continue
            };
        ...
        for patch in &patch_set.patches {
            ...
            if self.entries.contains_key(&path) {
                ...
                match bsdiff::patch(old_text.as_slice(), &mut bsdiff_patch, &mut new_text) {
                    ...
                    Err(e) => {
                        println!("Failed to apply patch {:?}. Is it generated by bsdiff-rs?", &patch.path);
                    }
                }
